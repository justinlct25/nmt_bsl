...
class InceptionI3d(nn.Module):
    ...
    # x: (B, C, T, H, W)
    # Batch Size (B): Number of video clips in the batch.
    # Channels (C): Number of input channels, which would typically be 3 for RGB videos.
    # Time Frames (T): Number of frames in each video clip.
    # Height (H): Height of each frame.
    # Width (W): Width of each frame.
    def extract_features(self, x):  ##tin## from bsl1k
        ...
        ##tin## bsl1k:
        last_duration = int(math.ceil(x.size(2) / 8))  # 8
        last_size = 7  # int(math.ceil(sample_width / 32))  # this is for 224
        avgpool = nn.AvgPool3d((last_duration, last_size, last_size), stride=1)
        embds = self.dropout(avgpool(x))
