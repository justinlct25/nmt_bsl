...
import i3d_wlasl.videotransforms as videotransforms

import numpy as np

import torch.nn.functional as F
from i3d_wlasl.pytorch_i3d import InceptionI3d

# from nslt_dataset_all import NSLT as Dataset
from i3d_wlasl.datasets.nslt_dataset_all import NSLT as Dataset
import cv2
...
#         frame_filename = f"/Users/chuntingjustinlo/Study/CardiffComputing/DissertationSignify/self_bslcp_preprocess/data_formating_slt_wicv2023_bslcp/preprocess_fp/i3d_wlasl/test_imgs/frame_{offset:04d}.jpg"
#         img_converted = ((img + 1) * 255 / 2).astype(np.uint8)
#         cv2.imwrite(frame_filename, cv2.cvtColor(img_converted, cv2.COLOR_RGB2BGR))

#     return torch.Tensor(np.asarray(frames, dtype=np.float32))

def load_rgb_frames_from_video(video_path, start=0, num=-1, center_square_crop=False):
    vidcap = cv2.VideoCapture(video_path)
    ...
    for offset in range(num):
        ...
        if center_square_crop:
            ...
            img = cv2.resize(img, dsize=(224, 224))  # Resize to 224x224
        else:
            sc = 224 / w
            img = cv2.resize(img, dsize=(0, 0), fx=sc, fy=sc)  
