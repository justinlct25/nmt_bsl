...
def rearrange(tsv_file_path, dataset_directory="./", split_ratios=[0.85, 0.07, 0.08], min_words=0):
    dataset_list = []
    ...
    with open(tsv_file_path, "r") as tsv_file:
        ...
        print(f"Start processing {total_lines} sentences...")
        for line_no, line in enumerate(tsv_file):
            ...
            if len(fields) == len(header):
                ...
                print(f"Appended sentence sign data {sentence_id} ({line_no}/{total_lines}lines)", end="\r")
            else:
    ...
    split_save_dataset(dataset_list, split_ratios, dataset_folder=dataset_folder_path)
...
def main():
    ...
    dataset_directory = './dataset_preprocess_output_stochastic2021'
    rearrange(tsv_file_path, dataset_directory=dataset_directory, min_words=9)
